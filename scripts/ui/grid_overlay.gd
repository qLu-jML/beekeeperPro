...
@onready var tilemap_ref: TileMap = get_node_or_null("../TileMap")
...
func _draw() -> void:
	...
	if show_grid:
		# Draw zone tiles first (behind everything else)
		if zone_manager:
			for tile in zone_manager.flower_tiles:
				var r = Rect2(tilemap.map_to_local(tile) - Vector2(8, 8), Vector2(16, 16))
				draw_rect(r, Color(0.2, 0.9, 0.2, 0.3), true)
				draw_rect(r, Color(0.2, 0.9, 0.2, 1.0), false, 1.0)
			for tile in zone_manager.apiary_tiles:
				var r = Rect2(tilemap.map_to_local(tile) - Vector2(8, 8), Vector2(16, 16))
				draw_rect(r, Color(0.9, 0.6, 0.1, 0.25), true)
				draw_rect(r, Color(0.9, 0.6, 0.1, 1.0), false, 1.0)
